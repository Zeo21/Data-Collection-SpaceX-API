...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        # Contar éxitos y fracasos (ocurrencias)
        counts = (
            filtered_df['class']
            .value_counts()
            .rename_axis('class')
        ...
        # Asegurarnos de que siempre existan las dos categorías (aunque alguna valga 0)
        counts = counts.set_index('class').reindex(['Success', 'Failure'], fill_value=0).reset_index()
        fig = px.pie(
            counts,
            values='count',
            names='class',
